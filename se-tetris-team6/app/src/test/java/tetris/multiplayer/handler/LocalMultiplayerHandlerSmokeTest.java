 ...
 *
 * 역할 요약:
 * - 로컬 2P 게임 진행을 관리하고 승패를 판정합니다.
 *
 * 테스트 전략:
 * - MultiPlayerGame/Controller/GameModel을 lenient mock으로 준비해 enter/update/exit를 호출하며
 *   예외 없이 통과하는지 확인합니다.
 */
...
class LocalMultiplayerHandlerSmokeTest {

    @Test
    void enterUpdateExit_smoke() {
        MultiPlayerGame game = mock(MultiPlayerGame.class, Mockito.withSettings().lenient());
        LocalMultiPlayerController controller = mock(LocalMultiPlayerController.class, Mockito.withSettings().lenient());
